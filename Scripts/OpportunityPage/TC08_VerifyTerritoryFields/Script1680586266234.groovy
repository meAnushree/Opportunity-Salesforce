...
'TC174.01 Get value from excel file'
user = CustomKeywords.'readDataFromDataFile.readData.search'('Data Files/Profiles/ProfileName', 'Sales Operations Clone', 'Type')

salesOp = findTestData('Profiles/ProfileName').getValue('Profile', user)

'Clicking on `Sytem Administrator Custom` profile '
WebUI.callTestCase(findTestCase('Common/TC_Impersonation'), [('profileList') : salesOp], FailureHandling.STOP_ON_FAILURE)
...
oportunityName = WebUI.getText(findTestObject('Sales/Opportunity/EditOpportunityPage/NewOpportunityCreated'))
